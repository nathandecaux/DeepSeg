        ...
        if x_u != None:
            ...
            if self.ssl_args['cons']:
                ...
                output_dict['sx_u_t'] = self.net_model.segmentor(
                    self.net_model.feature(self.norm(x_u_t)))   
    ...
    def compute_loss(self, x, y, x_u, output_dict):
        ...
        if x != None:
            ...
            if self.criterion=='w-tversky':
                    ...
                    y_hat=nn.Softmax()(output_dict["sx"])

                    PG=(y_hat[:,1,...]*y_oh[:,1,...]).mean()
                    P_G=(y_hat[:,1,...]*y_oh[:,0,...]).mean()
                    G_P=(y_oh[:,1,...]*y_hat[:,0,...]).mean()
                    l_seg=1-PG/(PG+nn.Softmax()(self.tversky_hp)[0]*P_G+nn.Softmax()(self.tversky_hp)[1]*G_P+1e-8)
                    
                    self.log('alpha',nn.Softmax()(self.tversky_hp)[0])
                    self.log('beta',nn.Softmax()(self.tversky_hp)[1])
            else :
                l_seg = ko.losses.dice_loss(output_dict["sx"], y.long())
            loss['seg'] = l_seg
        ...
        if x_u != None:
            ...
            if self.ssl_args['cons']:
                if self.criterion=='w-tversky':
                    ...
                    P_G=(y_hat[:,1,...]*y_oh[:,0,...]).mean()
                    G_P=(y_oh[:,1,...]*y_hat[:,0,...]).mean()
                    l_cons=1-PG/(PG+nn.Softmax()(self.tversky_hp)[0]*P_G+nn.Softmax()(self.tversky_hp)[1]*G_P+1e-8)
                    
                else :
                    l_cons = ko.losses.dice_loss(output_dict["sx_u_t"], torch.argmax(output_dict["sx_u"], 1).long())
                loss['cons'] = l_cons
